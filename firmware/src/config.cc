    ...
    if (report_id == REPORT_ID_CONFIG && bufsize >= CONFIG_SIZE) {
        if (checksum_ok(buffer, CONFIG_SIZE) && command_version_ok(buffer)) {
            ...
            switch (config_buffer->command) {
                ...
                case (ConfigCommand) COMMAND_INJECT_INPUT: {
                    struct injected_input_packet_t {
                        int16_t x;
                        int16_t y;
                        uint8_t buttons;
                    } __attribute__((packed));
                    injected_input_packet_t* packet = (injected_input_packet_t*) config_buffer->data;
                    injected_input.x += packet->x;
                    injected_input.y += packet->y;
                    injected_input.buttons = packet->buttons;
                    injected_input_updated = true;
                    break;
                }
                default:
